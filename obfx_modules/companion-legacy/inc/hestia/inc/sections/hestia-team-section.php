			...
			if ( ! empty( $icons_decoded ) ) {
				...
				foreach ( $icons_decoded as $value ) {
					...
					if ( ! empty( $social_icon ) ) {
						...
						$aria_label = apply_filters( 'hestia_team_social_media_aria_text', $aria_label );
						$link = '<a href="' . esc_url( $social_link ) . '"';
						if ( function_exists( 'hestia_is_external_url' ) ) {
							$link .= hestia_is_external_url( $social_link );
						}
						$link .= ' class="btn btn-just-icon btn-simple" aria-label="' . $aria_label . '"><i class="' . esc_attr( hestia_display_fa_icon( $social_icon ) ) . '"></i></a>';
						echo $link;
					}
				}
				echo '</div>';
