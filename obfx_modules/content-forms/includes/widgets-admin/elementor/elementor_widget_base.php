...
abstract class Elementor_Widget_Base extends Widget_Base {
	...
	protected function render() {
		...
		echo '<button type="submit" name="submit" ' . esc_attr( $disabled ) . ' value="submit-' . esc_attr( $this->get_widget_type() ) . '-' . esc_attr( $form_id ) . '" class="' . esc_attr( $this->get_render_attribute_string( 'button' ) ) . '">';
		echo esc_html( $btn_label );

		if ( isset( $settings['button_icon_new'] ) ) {
			echo '<span ' . esc_attr( $this->get_render_attribute_string( 'content-wrapper' ) ) . '>';
			echo '<span ' . esc_attr( $this->get_render_attribute_string( 'icon-align' ) ) . '>';
			...
			foreach ( $form_fields as $field ) {
				...
				if ( in_array( $field_map, $mapping, true ) ) {
					...
					printf(
						...
						'<strong>' . esc_html( $field_map ) . '</strong>'
					);
					echo '</p>';
					$has_error = true;
				}
	...
	protected function maybe_load_widget_style() {
		if ( Plugin::$instance->editor->is_edit_mode() === true && apply_filters( 'themeisle_content_forms_register_default_style', true ) ) {
			echo '<style>';
			echo esc_html( $this->get_notice_style() );
			echo '</style>';
		} else {
	...
	private function maybe_render_form_notification() {
		...
		echo '<h3 ' . esc_attr( $style ) . ' class="content-form-notice content-form-success">' . esc_html__( 'This is a preview of how the success notification will look', 'themeisle-companion' ) . '</h3>';
		echo '</div>';

		echo '<div class="content-form-notice-wrapper">';
		echo '<h3 ' . esc_attr( $style ) . ' class="content-form-notice content-form-error">' . esc_html__( 'This is a preview of how the error notification will look', 'themeisle-companion' ) . '</h3>';
		echo '</div>';
	...
	private function render_form_field( $field, $is_preview = false ) {
		...
		echo '<fieldset ' . esc_attr( $this->get_render_attribute_string( 'fieldset' . $field_id ) ) . '>';
		$this->render_field_label( $field );

		switch ( $field['type'] ) {
			case 'textarea':
				echo '<textarea name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_name ) . '" ' . esc_attr( $disabled ) . ' ' . esc_attr( $required ) . ' placeholder="' . esc_attr( $placeholder ) . '" cols="30" rows="5"></textarea>';
				break;
			case 'password':
				echo '<input type="password" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_name ) . '" ' . esc_attr( $required ) . ' ' . esc_attr( $disabled ) . '>';
				break;
			case 'hidden':
				$hidden_field_value = $field['hidden_value'];
				echo '<input type="hidden" value="' . esc_attr( $hidden_field_value ) . '" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_name ) . '" ' . esc_attr( $disabled ) . '>';
				break;
			case 'checkbox':
				echo '<label class="checkbox-input" for="' . esc_attr( $field_name ) . '" ' . esc_attr( $this->get_render_attribute_string( 'label' . $field_id ) ) . '>';
				echo wp_kses_post( $field['label'] );
				if ( $field['requirement'] === 'required' ) {
					echo '<span class="required-mark"> *</span>';
				}
				echo '<input type="checkbox" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_name ) . '" ' . esc_attr( $required ) . ' ' . esc_attr( $disabled ) . '>';
				echo '</label>';
				break;
			default:
				echo '<input type="text" name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $field_name ) . '" ' . esc_attr( $required ) . ' ' . esc_attr( $disabled ) . ' placeholder="' . esc_attr( $placeholder ) . '">';
				break;
		}
		echo '</fieldset>';
	...
	private function render_field_label( $field ) {
		...
		echo '<label for="' . esc_attr( $field_name ) . '" ' . esc_attr( $this->get_render_attribute_string( 'label' . $field_id ) ) . '>';
	 ...
	 * @return string
	 */
	abstract public function get_widget_type();
	 ...
	 */
	abstract public function add_repeater_specific_fields( $repeater );
	 ...
	 * @return mixed
	 */
	abstract public function add_widget_specific_settings();
	 ...
	 */
	abstract public function add_specific_form_fields();

	/**
	 * Add widget specific settings controls.
	 */
	abstract public function add_specific_settings_controls();
	 ...
	 * @return array
	 */
	abstract public function get_specific_field_types();
}
