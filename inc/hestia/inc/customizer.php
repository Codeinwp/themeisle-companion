...
if ( ! function_exists( 'hestia_companion_customize_register' ) ) :
	...
	function hestia_companion_customize_register( $wp_customize ) {
		...
		if ( ! empty( $hestia_features_title_control ) ) {
			$hestia_features_title_control->default = esc_html__( 'Why our product is the best', 'hestia-companion' );
		}
		...
		if ( ! empty( $hestia_features_subtitle_control ) ) {
			$hestia_features_subtitle_control->default = esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'hestia-companion' );
		}
		...
		if ( ! empty( $hestia_team_title_control ) ) {
			$hestia_team_title_control->default = esc_html__( 'Meet our team', 'hestia-companion' );
		}
		$hestia_team_subtitle_control = $wp_customize->get_setting( 'hestia_team_subtitle' );
		if ( ! empty( $hestia_team_subtitle_control ) ) {
			$hestia_team_subtitle_control->default = esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'hestia-companion' );
		}
		...
		if ( ! empty( $hestia_team_content_control ) ) {
			$hestia_team_content_control->default = json_encode( array(
				...
				'title'           => esc_html__( 'Desmond Purpleson', 'hestia-companion' ),
				'subtitle'        => esc_html__( 'CEO', 'hestia-companion' ),
				'text'            => esc_html__( 'Locavore pinterest chambray affogato art party, forage coloring book typewriter. Bitters cold selfies, retro celiac sartorial mustache.', 'hestia-companion' ),
				...
				'title'           => esc_html__( 'Parsley Pepperspray', 'hestia-companion' ),
				'subtitle'        => esc_html__( 'Marketing Specialist', 'hestia-companion' ),
				'text'            => esc_html__( 'Craft beer salvia celiac mlkshk. Pinterest celiac tumblr, portland salvia skateboard cliche thundercats. Tattooed chia austin hell.', 'hestia-companion' ),
				...
				'title'           => esc_html__( 'Desmond Eagle', 'hestia-companion' ),
				'subtitle'        => esc_html__( 'Graphic Designer', 'hestia-companion' ),
				'text'            => esc_html__( 'Pok pok direct trade godard street art, poutine fam typewriter food truck narwhal kombucha wolf cardigan butcher whatever pickled you.', 'hestia-companion' ),
				...
				'title'           => esc_html__( 'Ruby Von Rails', 'hestia-companion' ),
				'subtitle'        => esc_html__( 'Lead Developer', 'hestia-companion' ),
				'text'            => esc_html__( 'Small batch vexillologist 90\'s blue bottle stumptown bespoke. Pok pok tilde fixie chartreuse, VHS gluten-free selfies wolf hot.', 'hestia-companion' ),
		...
		if ( ! empty( $hestia_testimonials_title_setting ) ) {
			$hestia_testimonials_title_setting->default = esc_html__( 'What clients say', 'hestia-companion' );
		}
		...
		if ( ! empty( $hestia_testimonials_subtitle_setting ) ) {
			$hestia_testimonials_subtitle_setting->default = esc_html__( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'hestia-companion' );
		}
